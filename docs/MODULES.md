...
> **설계 변경 (2026-06-03)**: 선형 다방 구조 → 단일 오픈맵으로 전면 교체.
> 스펙: `docs/superpowers/specs/2026-06-03-dungeon-rework-design.md`

> **모듈 완료 테스트**
> 1. ManagementScene 포털 진입 → DungeonScene 전환
> 2. 몬스터 스폰 확인, 처치 후 8초 리스폰 확인
> 3. 재료 픽업 → 가방 무게 증가 확인
> 4. 가방 꽉 참 → 픽업 거부 + 알림 확인
> 5. B키 가방 UI → 재료 버리기 → 무게 감소 확인
> 6. DungeonExit E키 (가방 미달) → 확인 팝업 → 귀환 → PlayerInventory 전송 확인
> 7. 미니맵 플레이어 도트 + 스폰 존 마커 확인
> 8. 플레이어 사망 → 1.5초 후 가방 전체 소실 + ManagementScene 복귀 확인
> 9. ResourceNode 공격 → 내구도 차감 → 0 도달 시 재료 드롭 + 리스폰 타이머 확인

| # | 항목 | 상태 |
|---|---|---|
| 5-1 | 타일맵 단일 맵 (벽 충돌, 카메라 경계) | ⚠️ BoxCollider2D 벽으로 경계 구현, 타일맵 미페인트 (에셋 미준비) |
| 5-2 | ~~다방 구조 & 방 간 이동~~ → **DungeonMapController + DungeonSpawnZone** (단일 맵 + 리스폰) | ✅ |
| 5-3 | 몬스터 스폰 포인트 & SpawnTable SO | ✅ (DungeonSpawnZone에서 재사용) |
| 5-4 | 던전 귀환 (DungeonExit 항상 활성, 가방 미달 확인 팝업, FlushToInventory) | ✅ |
| 5-5 | DungeonBag (무게 기반 임시 컨테이너, 버리기 UI, 가방 HUD 바) | ✅ |
...
> ~~2. 인벤토리 슬롯 간 아이템 드래그 & 드롭 이동~~ (제거 — 클릭 기반 UI로 충분)
> ~~3. 요리 재료 슬롯으로 아이템 드래그 가능~~ (제거 — CookingUI 클릭 방식 유지)
...
## 모듈 7 — 요리 시스템 ✅ 완료 (7-5 VFX 에셋 미제작 — 아트 작업 후 연동)
...
| 8-2 | 메뉴 등록 UI (FoodInventory → 메뉴판 슬롯, 가격 설정) | ✅ |
...
| 8-8 | 손님 만족도 (Patience 타이머 → 만족도 점수 계산) | ✅ |
| 8-9 | 테이블 청소 (퇴장 후 오염 마크, 플레이어 청소 상호작용) | ✅ |
| 8-10 | 영업 정산 화면 (수입 / 손님 수 / 만족도 요약 UI) | ✅ |
...
| # | 항목 | 상태 |
|---|---|---|
| 9-0 | StartScene — GlobalController + GameStartup (4단계 Startup 파이프라인) + StartSceneController (Touch To Start) | ✅ |
...
## 모듈 11 — ManagementScene 기지 허브 ✅ 완료

> **범위 기준**: ManagementScene에서 플레이어가 수행하는 모든 행동.
> 던전 포털 진입 / 저녁 시작 / 업그레이드 구매.
> 씬 전환 자체(SceneLoader)는 모듈 9에 포함.

> **모듈 완료 테스트**
> 1. ManagementScene 첫 진입 → 플레이어 스폰 + 이동 정상 동작
> 2. DungeonPortal E키 → DungeonScene 전환 확인
> 3. EveningStarter E키 → KitchenScene 전환 확인
> 4. ToolUpgradeStation E키 → 비용·현재 레벨 HUD 알림 → 골드 차감 + 스탯 반영 확인
> 5. ShopUpgradeStation E키 → 비용·현재 레벨 HUD 알림 → 골드 차감 + 손님 수 반영 확인
> 6. 골드 부족 시 "골드 부족" 알림 표시 (DebugUtil 아닌 GameHUD.ShowNotification)

| # | 항목 | 상태 |
|---|---|---|
| 11-1 | ManagementSceneController — 플레이어 스폰(첫 진입) / 재배치(복귀) | ✅ |
| 11-2 | DungeonPortal — E키 → DungeonScene 전환 | ✅ |
| 11-3 | EveningStarter — E키 → KitchenScene 전환 | ✅ |
| 11-4 | 업그레이드 스테이션 피드백 — GameHUD.ShowNotification 으로 비용·레벨 표시 (Trigger 진입 시 힌트 + 성공/실패 알림) | ✅ |
| 11-5 | BagUpgradeStation — 가방 용량 업그레이드 스테이션 씬 배치 (position 4,2,0) | ✅ |
...
| # | 항목 | 상태 |
|---|---|---|
| 12-1 | AudioManager 싱글톤 (BGM 채널 + SFX 채널 분리, 볼륨 독립 조절) | ❌ |
| 12-2 | 던전 BGM / 주방 BGM / 식당 BGM 씬 연동 | ❌ |
| 12-3 | 전투 SFX (히트·사망·스킬) | ❌ |
| 12-4 | UI SFX (버튼·팝업·알림) | ❌ |
| 12-5 | 옵션 패널 볼륨 슬라이더 | ❌ |

---

## 모듈 13 — 음식 버프 시스템 (Post-MVP)

> **우선순위**: 식당 시스템 안정화 후.

| # | 항목 | 상태 |
|---|---|---|
| 13-1 | FoodData 버프 필드 (buffType·buffValue·buffDuration) | ❌ |
| 12-2 | 요리 등급별 버프 배율 (Normal/Good/Perfect) | ❌ |
| 13-3 | 플레이어 버프 컴포넌트 (스탯 임시 증가, 타이머) | ❌ |
| 13-4 | 식사 → 버프 적용 흐름 (Customer.EatAndPay → PlayerBuff) | ❌ |
| 13-5 | HUD 버프 아이콘 + 잔여 시간 표시 | ❌ |

---

## 모듈 14 — 튜토리얼 & 온보딩 (Post-MVP)

> **우선순위**: 전체 사이클 플레이테스트 후.

| # | 항목 | 상태 |
|---|---|---|
| 14-1 | 첫 실행 감지 (IsFirstPlay 플래그, ServerSaveData) | ❌ |
| 14-2 | 이동 튜토리얼 (화살표 오버레이 + WASD 입력 감지) | ❌ |
| 14-3 | 던전 채집 튜토리얼 (적 처치 → 재료 줍기) | ❌ |
| 14-4 | 주방 요리 튜토리얼 (레시피 슬롯 가이드) | ❌ |
| 14-5 | 식당 서빙 튜토리얼 (손님 등장 → E키 서빙) | ❌ |

---

## 모듈 15 — 농장 시스템 (Post-MVP)

> **우선순위**: 모든 핵심 루프 안정화 후. FarmManager·FarmPlot 기초 코드 존재.

| # | 항목 | 상태 |
|---|---|---|
| 15-1 | FarmPlot 심기 / 성장 타이머 / 수확 (기초 구현 존재) | ⚠️ 기초 코드만 |
| 15-2 | FarmManager — 전체 농장 플롯 관리 | ⚠️ 기초 코드만 |
| 15-3 | 씨앗 데이터 SO (SeedData — 성장 시간·결과 재료) | ❌ |
| 15-4 | ManagementScene 농장 구역 배치 + 상호작용 | ❌ |
| 15-5 | FarmPlot UI (성장 진행 바, 수확 알림) | ❌ |
...
모듈 1  기반 인프라          ✅ 완료
모듈 2  조작                ✅ 완료
모듈 3  전투                🔄 진행 중 (3-A6 실 클립 연결, 3-A7 무기 에셋 — 아트 작업 후)
모듈 4  몬스터 AI            ✅ 완료
모듈 5  던전                ✅ 완료
모듈 6  인벤토리             ✅ 완료
모듈 7  요리 시스템          ✅ 완료 (7-5 VFX 에셋 제외)
모듈 8  식당 시스템          ✅ 완료
모듈 9  씬 흐름 & HUD        ✅ 완료
모듈 10 성능 최적화          ✅ 완료
모듈 11 ManagementScene 허브 ✅ 완료
─────────────────────────── MVP 경계선 ───────────────────────────
모듈 12 오디오               ❌ Post-MVP
모듈 13 음식 버프 시스템      ❌ Post-MVP
모듈 14 튜토리얼 & 온보딩     ❌ Post-MVP
모듈 15 농장 시스템           ❌ Post-MVP (기초 코드만)
```
