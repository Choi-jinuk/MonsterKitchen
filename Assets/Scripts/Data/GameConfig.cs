...
namespace MonsterKitchen.Data
{
    ...
    //
    //  ▶ 런타임 튜닝 (향후)
    //    GameConfig.Current 의 필드를 직접 수정하면
    //    [ExecuteAlways] 로 동작하는 PerspectiveMapTilt · PerspectivePlaneSync 가
    //    즉시 반영한다. 빌드에서는 인메모리 변경만 되고 에셋 파일은 수정되지 않는다.
    //    별도 디버그 UI 슬라이더 패널에서 Current 필드를 노출하면
    //    실시간 튜닝 워크플로우가 완성된다.
    //
    //  ▶ 에셋 위치
    ...
    public class GameConfig : ScriptableObject
    {
        ...
        /// </summary>
        public static GameConfig Current => _current;
        ...
        [Tooltip("맵 기울기 각도 (도).\n" +
                 "PerspectiveMapTilt: 배경 GO X 회전값.\n" +
                 "PerspectivePlaneSync: Z = Y * tan(angle).")]
        [Range(0f, 89f)]
        public float tiltAngleDeg = 5f;
