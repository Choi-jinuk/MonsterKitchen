...
## 진행 중: 오크 변종 몬스터 추가 (ContinuousMovement) (2026-06-14)

### 지시 (2026-06-14)
- 슬라임 말고 Continuous 동작 사용하는 오크 몬스터 더 만들어 던전에 배치.
- 결정: 변종 2종(1007/1008) 신규 + 기존 Orc(1006)도 던전 배치. 전용 스킬그룹 신규.

### 배경
- 기존 Orc(MON_006/1006): ContinuousMovement 프리팹·매니페스트·CSV 존재하나 **스폰 테이블 미등록**.
- 모든 몬스터 BT 는 단일 `MonsterBT.asset`(데이터 구동) 공유. 슬라임/오크 프리팹·스프라이트도 공유.
- → "신규 BT" = 신규 SkillGroup/SkillStep + 매니페스트 키. 새 에셋 파일 불필요.

### 변종 스탯
| ID | 이름 | HP | ATK | DEF | SPD | 속성 | 등급 | 스킬 | 드롭 | 특이 |
|----|------|----|----|----|----|----|----|----|----|----|
| 1006 | 오크 졸병 | 50 | 7 | 3 | 3.0 | None | Common | SGD_101 | 5001 | 기존 |
| 1007 | 오크 전사 | 90 | 12 | 10 | 2.8 | Earth | Uncommon | SGD_106 | 5004 | 넉백 면역, 강타 |
| 1008 | 오크 광전사 | 70 | 15 | 6 | 3.6 | Fire | Rare | SGD_107 | 5005 | 빠름, 돌진 |

### 단계
- [x] 1. Monsters.csv — MON_007/008 추가
- [x] 2. SkillGroups.csv — SGD_106/107
- [x] 3. SkillSteps.csv — SKL_M06(11306)/SKL_M07(11307)
- [x] 4. StringData.csv — MON_006/007/008 NAME·DESC (8068~8073)
- [x] 5. DungeonSpawnTables.csv — 활성 존 6003/6004/6005 에 1006/1007/1008 배치
- [x] 6. OrcVariantsAutomation.cs — SyncAllSO + 매니페스트 1007/1008 등록
- [x] 7. MCP 메뉴 실행(에디터 열림→batchmode 대신) → SO 동기화 + 매니페스트 + Localization Import
- [x] 8. read_console 확인 — 컴파일/유효성 통과, 에러 0 (MCP 재접속 blip 제외)
- [x] 9. 완료 보고

### 처리 결과 (2026-06-14)
- Sync All: 항목 87개, Monster 8개, 유효성 검사 통과 ✓
- 매니페스트 1007/1008 prefab(Orc)/sprite(Slime)/bt(MonsterBT) 등록 ✓
- Localization 73키 임포트 ✓
- 모든 오크(1006/1007/1008) ContinuousMovement·Orc 프리팹 공유, 스프라이트는 Slime 재사용(전용 오크 아트 없음)
- 던전 활성 존 배치: 6003→1006×2, 6004→1007×1, 6005→1008×1
